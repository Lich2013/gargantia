...
public class TabletopInitialization : MonoBehaviour {
	private const string typeName = "DunderJam";
	public string gameName = "Dunder Jam";
	...
	[HideInInspector]
	public string appID = "DunderJam";							//name of game
	[HideInInspector]
	...
	public IEnumerator MeteorInit() {
		...
		createMsgLog ("...Past Meteor.connect");
		...
		playerCollection.DidAddRecord += (string arg1, PlayerTemplate arg2) => {
			Debug.Log ("player collection got record added, name: " + arg2.name + ", color: " + arg2.color + "playerID: " + arg2.playerID + ", _id: " + arg2._id + "sessionID" + arg2.session_id);
	...
	IEnumerator callUpdatePlayer(string playerName, string clr) {
		//string theColor = clr.ToString ();
		yield return new WaitForSeconds (0.1f);
		Debug.Log ("in callUpdatePlayer, name: " + playerName +", color: " + clr);
		var methodCall = Meteor.Method<ChannelResponse>.Call ("updatePlayer", playerName, clr);
